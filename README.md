...
## 開発手法と安全に関する注記
本プロジェクトは、高専PBL課題の一環として、LLMを活用した迅速なプロトタイピング手法（いわゆるVibe Coding）を産業用ロボット制御に適用する実験的試みです。

・開発プロセス: 開発サイクルの極大化を優先し、コードベースの大半をAIとの協調プロンプティングによって生成しています。

・品質と挙動について: 短期間での機能実証を最優先したため、一部リファクタリングが未完了な箇所やコードの冗長性が含まれています。

・実機検証時の注意: 実機（duAro）での動作検証にあたっては、速度リミットの設定および手動非常停止の確保など、予期せぬ軌道生成に対する安全対策を講じた上で実行してください。
